...
async def list_dunning_campaigns(
    ...
    campaigns = repo.get_all(
        organization_id, skip=skip, limit=limit, order_by=order_by, status=status
    )
...
@router.get(
    "/{campaign_id}/execution_history",
    response_model=list[ExecutionHistoryEntry],
...
async def get_execution_history(
    ...
    """Get payment requests generated by this campaign with customer and invoice details."""
    repo = DunningCampaignRepository(db)
    campaign = repo.get_by_id(campaign_id, organization_id)
    if not campaign:
        raise HTTPException(status_code=404, detail="Dunning campaign not found")
    return repo.execution_history(campaign_id, organization_id, skip=skip, limit=limit)
...
async def get_campaign_timeline(
    ...
    if not campaign:
        raise HTTPException(status_code=404, detail="Dunning campaign not found")
    return repo.campaign_timeline(campaign_id, organization_id)


@router.post(
    "/{campaign_id}/preview",
    response_model=CampaignPreviewResponse,
    summary="Preview campaign execution",
    responses={
        401: {"description": "Unauthorized"},
        404: {"description": "Dunning campaign not found"},
    },
...
async def preview_campaign(
    ...
    repo = DunningCampaignRepository(db)
    result = repo.campaign_preview(campaign_id, organization_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Dunning campaign not found")
    return result
