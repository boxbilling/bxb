...
- [x] Add notification system
  <!-- Completed: Added full in-app notification system. Backend: Notification model with category, title, message, resource_type/resource_id, is_read fields. NotificationRepository with CRUD, count_unread, mark_as_read, mark_all_as_read. NotificationService with typed helper methods for 5 notification categories: webhook failures (notify_webhook_failure), dunning alerts (notify_dunning_alert), wallet expiry (notify_wallet_expiring), invoice overdue (notify_invoice_overdue), payment failures (notify_payment_failed). 4 API endpoints: GET /v1/notifications/ (list with category/is_read/order_by filters), GET /v1/notifications/unread_count, POST /v1/notifications/{id}/read, POST /v1/notifications/read_all. Integrated notification creation into WebhookService (on delivery failure — HTTP errors and network exceptions) and DunningService (on payment request creation with customer name lookup). Frontend: NotificationBell component in admin header with popover panel, real-time unread count badge (polls every 30s), notification list with category icons/colors, time-ago timestamps, mark-as-read per item, mark-all-as-read button, and click-to-navigate to related resource detail pages. Alembic migration for notifications table with indexes. 38 new backend tests (16 repository, 10 service, 2 schema, 10 API). All 4077 backend tests pass with 100% coverage. -->
- [x] Add activity feed improvements
  <!-- Completed: Expanded the dashboard activity feed from 4 to 8 event types: added subscription_canceled, payment_failed, credit_note_created, and wallet_topped_up alongside existing customer_created, subscription_created, invoice_finalized, and payment_received. Added resource_type and resource_id fields to RecentActivityResponse schema enabling clickable navigation from each activity item to the related resource's detail page (customers, subscriptions, invoices, wallets). Enriched descriptions with contextual info (payment amounts, credit note numbers, wallet credit amounts). Frontend: activity items are now clickable Links with hover styling when a detail page exists, new color-coded icons per type (green for payments, red for failures, orange for cancellations, purple for credit notes, blue for wallet top-ups), expanded filter dropdown with all 8 types, and 30-second live polling via refetchInterval for auto-refresh. 4 new backend repository methods (recent_canceled_subscriptions, recent_failed_payments, recent_credit_notes, recent_wallet_topups). 10 new backend tests added. All 4087 tests pass with 100% coverage. -->
- [x] Add charge model visualizer for plans
  <!-- Already completed in section 3.5 — ChargeModelVisualizer component exists at src/components/ChargeModelVisualizer.tsx with visual charts for graduated, volume, graduated percentage, standard, package, and percentage charge models. Integrated into PlanDetailPage Charges tab. -->
- [x] Add revenue analytics deep-dive page
  <!-- Completed: Added GET /dashboard/revenue_analytics backend endpoint aggregating 5 analytics dimensions: daily revenue trend (reuses existing daily_revenue repository method), revenue by invoice type (new revenue_by_invoice_type repo method with GROUP BY on invoice_type), top 10 customers by revenue (new top_customers_by_revenue repo method with Customer JOIN and ORDER BY revenue DESC), collection efficiency metrics (new collection_metrics repo method computing total invoiced/collected, collection rate, average days to payment with SQLite/PostgreSQL dialect support, and overdue invoice count/amount), and net revenue breakdown (new net_revenue repo method aggregating gross revenue minus refunded payments and finalized credit notes). New Pydantic schemas: DailyRevenuePoint, RevenueByTypeBreakdown, TopCustomerRevenue, CollectionMetrics, NetRevenueMetrics, RevenueAnalyticsResponse. Frontend RevenueAnalyticsPage.tsx at /admin/revenue-analytics with: period selector (7d/30d/90d/12m/custom date range), 4 summary stat cards (Net Revenue, Collection Rate with color coding, Avg Days to Payment, Overdue Amount), daily revenue AreaChart with gradient fill, revenue by type donut chart with legend and invoice counts, net revenue breakdown BarChart with gross/refunds/credits/net waterfall + summary grid, collection efficiency card with progress bar and 4 metric cells, and top customers table with rank/name (clickable link to customer detail)/revenue/invoice count/share percentage with progress bar. Added "Revenue Analytics" nav item (TrendingUp icon) in sidebar Overview group. Added "Deep dive" link in Dashboard revenue chart header. Frontend API client extended with dashboardApi.getRevenueAnalytics(). 23 new backend tests added (12 API + 11 repository) covering: empty DB, with data, date range filtering, narrow range exclusion, by-type breakdown, top customers ordering/limit, collection rate calculation, avg days to payment, overdue invoices, net revenue with refunds/credit notes, and PostgreSQL dialect branch. All 4108 backend tests pass with 100% coverage. -->
